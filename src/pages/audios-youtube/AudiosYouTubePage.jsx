...
const AudiosYouTubePage = () => {
  ...
  // Estado para polling de descargas (varias en simultáneo, una entrada por filename)
  // downloads[filename] = { progress, statusMessage, error, meta: { title, thumbnail, duration } }
  const downloadPollingRefs = useRef({});
  ...
  const clearDownloadPolling = useCallback((filename) => {
    if (downloadPollingRefs.current[filename]) {
      clearTimeout(downloadPollingRefs.current[filename]);
      delete downloadPollingRefs.current[filename];
    }
  }, []);
  ...
  /**
   * Iniciar polling de estado de descarga para un archivo específico.
  ...
  useEffect(() => {
    return () => {
      clearProcessPolling();
      Object.values(downloadPollingRefs.current).forEach(clearTimeout);
      downloadPollingRefs.current = {};
    };
  }, [clearProcessPolling]);
